...
def post_and_like_tweet(file_name, delay=30):
    try:
        ...
        for index, tweet in enumerate(tweets, start=1):
            ...
            if tweet_text:
                rest_id = "1884620681914278296"
                payload = {"variables": {"tweet_text": tweet_text+"#zelena #zeudi #helena #helevier #Tommavi #shailenzo #sstvidf #GrandeFratello #MAGA","reply": {"in_reply_to_tweet_id": rest_id,"exclude_reply_user_ids": []},"dark_request": False,"media": {"media_entities": [],"possibly_sensitive": False},"semantic_annotation_ids": [],"disallowed_reply_options": None},"features": CREATE_API_FEATURES,  "queryId": "BjT3MvG1CwfTuJxTLX4ovg" }
...
#                         post_tweets_for_ids(rest_id)
                        asyncio.run(post_tweets_for_ids(rest_id , tweet_text ))
...
# def post_tweets_for_ids(tweet_id):
...
#             payload_template["variables"]["tweet_text"] = f"{i}. that's true"
...
async def post_tweets_for_ids(tweet_id, tweet_text):
    print(f"Posting tweets for in_reply_to_tweet_id: {tweet_id}")

    for i in range(1, 10):
        # Update the tweet_text and in_reply_to_tweet_id for each iteration
        payload_template["variables"]["tweet_text"] = tweet_text+ "#zelena #zeudi #helena #helevier #Tommavi #shailenzo #sstvidf #GrandeFratello #MAGA"
        payload_template["variables"]["reply"]["in_reply_to_tweet_id"] = tweet_id
        # Convert payload to JSON
        payload = json.dumps(payload_template)
        ...
        # **Async delay of 20 seconds before next tweet**
        await asyncio.sleep(20)

# # Run the async function
# tweet_id = "your_tweet_id_here"  # Replace with actual tweet ID
# asyncio.run(post_tweets_for_ids(tweet_id))


# Run the script
if __name__ == "__main__":
    post_and_like_tweet(TWEET_FILE, delay=30)
