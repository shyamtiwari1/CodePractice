...
TWEET_FILE = '/Users/shyamtiwari/Library/Mobile Documents/com~apple~TextEdit/Documents/plainTextTweets.txt'
...
# Function to post and like tweet

def read_tweets(file_name):
    """Reads tweets from a file, treating *** as the separator for each tweet.
       Inserts a blank line between any two text lines of a single tweet."""
    with open(file_name, 'r') as file:
        content = file.read()

    # Split tweets by '***' while preserving multi-line format
    raw_tweets = content.split('***')
    tweets = [tweet.strip() for tweet in raw_tweets if tweet.strip()]  # Remove extra spaces & empty tweets

    # Insert a blank line between lines of each tweet
    formatted_tweets = ["\n".join(tweet.splitlines()) for tweet in tweets]

    return formatted_tweets

def post_and_like_tweet(file_name, delay=30):
    print(f"Posting tweets from file: {file_name}")
    try:
        tweets = read_tweets(file_name)  # Read tweets using *** separator
        for index, tweet in enumerate(tweets, start=1):
            print(f"Posting tweet {index}...")
            tweet_text = tweet
            if tweet_text:
                payload = {
                    "variables": {
                ...
                if response.status_code == 200:
                    ...
                    print(f"{index}: Failed to post tweet:\n{tweet_text}")
